...
<h2 id="_1-为什么使用索引" tabindex="-1"><a class="header-anchor" href="#_1-为什么使用索引"><span>1. 为什么使用索引</span></a></h2>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616141351236.png" alt="image-20220616141351236" tabindex="0" loading="lazy"><figcaption>image-20220616141351236</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616142723266.png" alt="image-20220616142723266" tabindex="0" loading="lazy"><figcaption>image-20220616142723266</figcaption></figure>
...
<h2 id="_2-索引及其优缺点" tabindex="-1"><a class="header-anchor" href="#_2-索引及其优缺点"><span>2. 索引及其优缺点</span></a></h2>
<h3 id="_2-1-索引概述" tabindex="-1"><a class="header-anchor" href="#_2-1-索引概述"><span>2.1 索引概述</span></a></h3>
<p>MySQL官方对索引的定义为：索引（Index）是帮助MySQL高效获取数据的数据结构。</p>
<p><strong>索引的本质</strong>：索引是数据结构。你可以简单理解为“排好序的快速查找数据结构”，满足特定查找算法。 这些数据结构以某种方式指向数据， 这样就可以在这些数据结构的基础上实现 <code v-pre>高级查找算法</code> 。</p>
<p><code v-pre>索引是在存储引擎中实现的</code>，因此每种存储引擎的索引不一定完全相同，并且每种存储引擎不一定支持所有索引类型。同时，存储引擎可以定义每个表的 <code v-pre>最大索引数</code>和 <code v-pre>最大索引长度</code>。所有存储引擎支持每个表至少16个索引，总索引长度至少为256字节。有些存储引擎支持更多的索引数和更大的索引长度。</p>
<h3 id="_2-2-优点" tabindex="-1"><a class="header-anchor" href="#_2-2-优点"><span>2.2 优点</span></a></h3>
<p>（1）类似大学图书馆建书目索引，提高数据检索的效率，降低 <strong>数据库的IO成本</strong> ，这也是创建索引最主 要的原因。</p>
...
<h2 id="_3-innodb中索引的推演" tabindex="-1"><a class="header-anchor" href="#_3-innodb中索引的推演"><span>3. InnoDB中索引的推演</span></a></h2>
<h3 id="_3-1-索引之前的查找" tabindex="-1"><a class="header-anchor" href="#_3-1-索引之前的查找"><span>3.1 索引之前的查找</span></a></h3>
<p>先来看一个精确匹配的例子：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>SELECT [列名列表] FROM 表名 WHERE 列名 = xxx;</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><h4 id="_1-在一个页中的查找" tabindex="-1"><a class="header-anchor" href="#_1-在一个页中的查找"><span>1. 在一个页中的查找</span></a></h4>
<p>假设目前表中的记录比较少，所有的记录都可以被存放到一个页中，在查找记录的时候可以根据搜索条件的不同分为两种情况：</p>
<ul>
<li>
<p>以主键为搜索条件</p>
<p>可以在页目录中使用 <code v-pre>二分法</code> 快速定位到对应的槽，然后再遍历该槽对用分组中的记录即可快速找到指定记录。</p>
</li>
<li>
<p>以其他列作为搜索条件</p>
<p>因为在数据页中并没有对非主键列简历所谓的页目录，所以我们无法通过二分法快速定位相应的槽。这种情况下只能从 <code v-pre>最小记录</code> 开始 <code v-pre>依次遍历单链表中的每条记录</code>， 然后对比每条记录是不是符合搜索条件。很显然，这种查找的效率是非常低的。</p>
</li>
</ul>
<h4 id="_2-在很多页中查找" tabindex="-1"><a class="header-anchor" href="#_2-在很多页中查找"><span>2. 在很多页中查找</span></a></h4>
...
<h3 id="_3-2-设计索引" tabindex="-1"><a class="header-anchor" href="#_3-2-设计索引"><span>3.2 设计索引</span></a></h3>
<p>建一个表：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>mysql> CREATE TABLE index_demo(</span></span>
...
<span class="line"><span>-> ) ROW_FORMAT = Compact;</span></span></code></pre>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616152453203.png" alt="image-20220616152453203" tabindex="0" loading="lazy"><figcaption>image-20220616152453203</figcaption></figure>
...
<p>把一些记录放到页里的示意图就是：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616152651878.png" alt="image-20220616152651878" tabindex="0" loading="lazy"><figcaption>image-20220616152651878</figcaption></figure>
<h4 id="_1-一个简单的索引设计方案" tabindex="-1"><a class="header-anchor" href="#_1-一个简单的索引设计方案"><span>1. 一个简单的索引设计方案</span></a></h4>
...
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>INSERT INTO index_demo VALUES(1, 4, 'u'), (3, 9, 'd'), (5, 3, 'y');</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div></li>
</ul>
<p>​       那么这些记录以及按照主键值的大小串联成一个单向链表了，如图所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616153518456.png" alt="image-20220616153518456" tabindex="0" loading="lazy"><figcaption>image-20220616153518456</figcaption></figure>
<p>​      从图中可以看出来， index_demo 表中的3条记录都被插入到了编号为10的数据页中了。此时我们再来插入一条记录</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>INSERT INTO index_demo VALUES(4, 4, 'a');</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>因为 <strong>页10</strong> 最多只能放3条记录，所以我们不得不再分配一个新页：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616155306705.png" alt="image-20220616155306705" tabindex="0" loading="lazy"><figcaption>image-20220616155306705</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616160216525.png" alt="image-20220616160216525" tabindex="0" loading="lazy"><figcaption>image-20220616160216525</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616160619525.png" alt="image-20220616160619525" tabindex="0" loading="lazy"><figcaption>image-20220616160619525</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616160857381.png" alt="image-20220616160857381" tabindex="0" loading="lazy"><figcaption>image-20220616160857381</figcaption></figure>
...
<h4 id="_2-innodb中的索引方案" tabindex="-1"><a class="header-anchor" href="#_2-innodb中的索引方案"><span>2. InnoDB中的索引方案</span></a></h4>
<h5 id="_1-迭代1次-目录项纪录的页" tabindex="-1"><a class="header-anchor" href="#_1-迭代1次-目录项纪录的页"><span>① 迭代1次：目录项纪录的页</span></a></h5>
<p>InnoDB怎么区分一条记录是普通的 <strong>用户记录</strong> 还是 <strong>目录项记录</strong> 呢？使用记录头信息里的 <strong>record_type</strong> 属性，它的各自取值代表的意思如下：</p>
<ul>
<li>0：普通的用户记录</li>
<li>1：目录项记录</li>
<li>2：最小记录</li>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616162944404.png" alt="image-20220616162944404" tabindex="0" loading="lazy"><figcaption>image-20220616162944404</figcaption></figure>
...
<h5 id="_2-迭代2次-多个目录项纪录的页" tabindex="-1"><a class="header-anchor" href="#_2-迭代2次-多个目录项纪录的页"><span>② 迭代2次：多个目录项纪录的页</span></a></h5>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616171135082.png" alt="image-20220616171135082" tabindex="0" loading="lazy"><figcaption>image-20220616171135082</figcaption></figure>
...
<h5 id="_3-迭代3次-目录项记录页的目录页" tabindex="-1"><a class="header-anchor" href="#_3-迭代3次-目录项记录页的目录页"><span>③ 迭代3次：目录项记录页的目录页</span></a></h5>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616173512780.png" alt="image-20220616173512780" tabindex="0" loading="lazy"><figcaption>image-20220616173512780</figcaption></figure>
...
<p>我们可以用下边这个图来描述它：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616173717538.png" alt="image-20220616173717538" tabindex="0" loading="lazy"><figcaption>image-20220616173717538</figcaption></figure>
<p>这个数据结构，它的名称是 B+树 。</p>
<h5 id="_4-b-tree" tabindex="-1"><a class="header-anchor" href="#_4-b-tree"><span>④ B+Tree</span></a></h5>
...
<h3 id="_3-3-常见索引概念" tabindex="-1"><a class="header-anchor" href="#_3-3-常见索引概念"><span>3.3 常见索引概念</span></a></h3>
<p>索引按照物理实现方式，索引可以分为 2 种：聚簇（聚集）和非聚簇（非聚集）索引。我们也把非聚集 索引称为二级索引或者辅助索引。</p>
<h4 id="_1-聚簇索引" tabindex="-1"><a class="header-anchor" href="#_1-聚簇索引"><span>1. 聚簇索引</span></a></h4>
<p>聚簇索引并不是一种单独的索引类型，而是<strong>一种数据存储方式</strong>（所有的用户记录都存储在了叶子结点），也就是所谓的 <code v-pre>索引即数据，数据即索引</code>。</p>
<blockquote>
...
<ul>
<li><code v-pre>页内</code> 的记录是按照主键的大小顺序排成一个 <code v-pre>单向链表</code> 。</li>
<li>各个存放 <code v-pre>用户记录的页</code> 也是根据页中用户记录的主键大小顺序排成一个 <code v-pre>双向链表</code> 。</li>
<li>存放 <code v-pre>目录项记录的页</code> 分为不同的层次，在同一层次中的页也是根据页中目录项记录的主键大小顺序排成一个 <code v-pre>双向链表</code> 。</li>
</ul>
</li>
<li>
<p>B+树的 叶子节点 存储的是完整的用户记录。</p>
<p>所谓完整的用户记录，就是指这个记录中存储了所有列的值（包括隐藏列）。</p>
</li>
</ul>
<p>我们把具有这两种特性的B+树称为聚簇索引，所有完整的用户记录都存放在这个<code v-pre>聚簇索引</code>的叶子节点处。这种聚簇索引并不需要我们在MySQL语句中显式的使用INDEX 语句去创建， <code v-pre>InnDB</code> 存储引擎会 <code v-pre>自动</code> 的为我们创建聚簇索引。</p>
<p><strong>优点：</strong></p>
<ul>
<li><code v-pre>数据访问更快</code> ，因为聚簇索引将索引和数据保存在同一个B+树中，因此从聚簇索引中获取数据比非聚簇索引更快</li>
<li>聚簇索引对于主键的 <code v-pre>排序查找</code> 和 <code v-pre>范围查找</code> 速度非常快</li>
<li>按照聚簇索引排列顺序，查询显示一定范围数据的时候，由于数据都是紧密相连，数据库不用从多 个数据块中提取数据，所以 <code v-pre>节省了大量的io操作</code> 。</li>
</ul>
<p><strong>缺点：</strong></p>
<ul>
<li><code v-pre>插入速度严重依赖于插入顺序</code> ，按照主键的顺序插入是最快的方式，否则将会出现页分裂，严重影响性能。因此，对于InnoDB表，我们一般都会定义一个<code v-pre>自增的ID列为主键</code></li>
<li><code v-pre>更新主键的代价很高</code> ，因为将会导致被更新的行移动。因此，对于InnoDB表，我们一般定义<strong>主键为不可更新</strong></li>
<li><code v-pre>二级索引访问需要两次索引查找</code> ，第一次找到主键值，第二次根据主键值找到行数据</li>
</ul>
<h4 id="_2-二级索引-辅助索引、非聚簇索引" tabindex="-1"><a class="header-anchor" href="#_2-二级索引-辅助索引、非聚簇索引"><span>2. 二级索引（辅助索引、非聚簇索引）</span></a></h4>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616203852043.png" alt="image-20220616203852043" tabindex="0" loading="lazy"><figcaption>image-20220616203852043</figcaption></figure>
<p>这个B+树与上边介绍的聚簇索引有几处不同：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616210404733.png" alt="image-20220616210404733" tabindex="0" loading="lazy"><figcaption>image-20220616210404733</figcaption></figure>
<p>**概念：回表 **</p>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616213109383.png" alt="image-20220616213109383" tabindex="0" loading="lazy"><figcaption>image-20220616213109383</figcaption></figure>
...
<li>一个表<code v-pre>只能有一个聚簇索引</code>，因为只能有一种排序存储的方式，但可以有<code v-pre>多个非聚簇索引</code>，也就是多个索引目录提供数据检索。</li>
<li>使用聚簇索引的时候，数据的<code v-pre>查询效率高</code>，但如果对数据进行插入，删除，更新等操作，效率会比非聚簇索引低。</li>
</ol>
<h4 id="_3-联合索引" tabindex="-1"><a class="header-anchor" href="#_3-联合索引"><span>3.联合索引</span></a></h4>
<p>我们也可以同时以多个列的大小作为排序规则，也就是同时为多个列建立索引，比方说我们想让B+树按 照 c2和c3列 的大小进行排序，这个包含两层含义：</p>
<ul>
<li>先把各个记录和页按照c2列进行排序。</li>
<li>在记录的c2列相同的情况下，采用c3列进行排序</li>
</ul>
<p>为c2和c3建立的索引的示意图如下：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220616215251172.png" alt="image-20220616215251172" tabindex="0" loading="lazy"><figcaption>image-20220616215251172</figcaption></figure>
<p>如图所示，我们需要注意以下几点：</p>
<ul>
...
<h3 id="_3-4-innodb的b-树索引的注意事项" tabindex="-1"><a class="header-anchor" href="#_3-4-innodb的b-树索引的注意事项"><span>3.4 InnoDB的B+树索引的注意事项</span></a></h3>
<h4 id="_1-根页面位置万年不动" tabindex="-1"><a class="header-anchor" href="#_1-根页面位置万年不动"><span>1. 根页面位置万年不动</span></a></h4>
<p>实际上B+树的形成过程是这样的：</p>
<ul>
<li>每当为某个表创建一个B+树索引（聚簇索引不是人为创建的，默认就有）的时候，都会为这个索引创建一个 <code v-pre>根结点</code> 页面。最开始表中没有数据的时候，每个B+树索引对应的 <code v-pre>根结点</code> 中即没有用户记录，也没有目录项记录。</li>
<li>随后向表中插入用户记录时，先把用户记录存储到这个<code v-pre>根节点</code> 中。</li>
<li>当根节点中的可用 <code v-pre>空间用完时</code> 继续插入记录，此时会将根节点中的所有记录复制到一个新分配的页，比如 <code v-pre>页a</code> 中，然后对这个新页进行 <code v-pre>页分裂</code> 的操作，得到另一个新页，比如<code v-pre>页b</code> 。这时新插入的记录根据键值（也就是聚簇索引中的主键值，二级索引中对应的索引列的值）的大小就会被分配到 <code v-pre>页a</code> 或者 <code v-pre>页b</code> 中，而 <code v-pre>根节点</code> 便升级为存储目录项记录的页。</li>
</ul>
...
<h4 id="_2-内节点中目录项记录的唯一性" tabindex="-1"><a class="header-anchor" href="#_2-内节点中目录项记录的唯一性"><span>2. 内节点中目录项记录的唯一性</span></a></h4>
<p>我们知道B+树索引的内节点中目录项记录的内容是 <code v-pre>索引列 + 页号</code> 的搭配，但是这个搭配对于二级索引来说有点不严谨。还拿 index_demo 表为例，假设这个表中的数据是这样的：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617151918786.png" alt="image-20220617151918786" tabindex="0" loading="lazy"><figcaption>image-20220617151918786</figcaption></figure>
<p>如果二级索引中目录项记录的内容只是 <code v-pre>索引列 + 页号</code> 的搭配的话，那么为 <code v-pre>c2</code> 列简历索引后的B+树应该长这样：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617152906690.png" alt="image-20220617152906690" tabindex="0" loading="lazy"><figcaption>image-20220617152906690</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617154135258.png" alt="image-20220617154135258" tabindex="0" loading="lazy"><figcaption>image-20220617154135258</figcaption></figure>
...
<h4 id="_3-一个页面最少存储-2-条记录" tabindex="-1"><a class="header-anchor" href="#_3-一个页面最少存储-2-条记录"><span>3. 一个页面最少存储 2 条记录</span></a></h4>
...
<h2 id="_4-myisam中的索引方案" tabindex="-1"><a class="header-anchor" href="#_4-myisam中的索引方案"><span>4. MyISAM中的索引方案</span></a></h2>
<p>B树索引使用存储引擎如表所示：</p>
<table>
<thead>
<tr>
...
<h3 id="_4-1-myisam索引的原理" tabindex="-1"><a class="header-anchor" href="#_4-1-myisam索引的原理"><span>4.1 MyISAM索引的原理</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617160413479.png" alt="image-20220617160413479" tabindex="0" loading="lazy"><figcaption>image-20220617160413479</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617160625006.png" alt="image-20220617160625006" tabindex="0" loading="lazy"><figcaption>image-20220617160625006</figcaption></figure>
...
<h3 id="_4-2-myisam-与-innodb对比" tabindex="-1"><a class="header-anchor" href="#_4-2-myisam-与-innodb对比"><span>4.2 MyISAM 与 InnoDB对比</span></a></h3>
...
<img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617161126022.png" alt="image-20220617161126022" style="float:left;" />
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617161151125.png" alt="image-20220617161151125" tabindex="0" loading="lazy"><figcaption>image-20220617161151125</figcaption></figure>
<h2 id="_5-索引的代价" tabindex="-1"><a class="header-anchor" href="#_5-索引的代价"><span>5. 索引的代价</span></a></h2>
...
<h2 id="_6-mysql数据结构选择的合理性" tabindex="-1"><a class="header-anchor" href="#_6-mysql数据结构选择的合理性"><span>6. MySQL数据结构选择的合理性</span></a></h2>
...
<h3 id="_6-1-全表查询" tabindex="-1"><a class="header-anchor" href="#_6-1-全表查询"><span>6.1 全表查询</span></a></h3>
<p>这里都懒得说了。</p>
<h3 id="_6-2-hash查询" tabindex="-1"><a class="header-anchor" href="#_6-2-hash查询"><span>6.2 Hash查询</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617162153587.png" alt="image-20220617162153587" tabindex="0" loading="lazy"><figcaption>image-20220617162153587</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617162604272.png" alt="image-20220617162604272" tabindex="0" loading="lazy"><figcaption>image-20220617162604272</figcaption></figure>
<p>上图中哈希函数h有可能将两个不同的关键字映射到相同的位置，这叫做 碰撞 ，在数据库中一般采用 链 接法 来解决。在链接法中，将散列到同一槽位的元素放在一个链表中，如下图所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617162703006.png" alt="image-20220617162703006" tabindex="0" loading="lazy"><figcaption>image-20220617162703006</figcaption></figure>
<p>实验：体会数组和hash表的查找方面的效率区别</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>// 算法复杂度为 O(n)</span></span>
<span class="line"><span>@Test</span></span>
<span class="line"><span>public void test1(){</span></span>
<span class="line"><span>    int[] arr = new int[100000];</span></span>
...
<span class="line"><span>    for(int j = 1; j&#x3C;=100000;j++){</span></span>
<span class="line"><span>        int temp = j;</span></span>
<span class="line"><span>        for(int i = 0;i &#x3C; arr.length;i++){</span></span>
<span class="line"><span>            if(temp == arr[i]){</span></span>
<span class="line"><span>                break;</span></span>
<span class="line"><span>            }</span></span>
<span class="line"><span>        }</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>    long end = System.currentTimeMillis();</span></span>
<span class="line"><span>    System.out.println("time： " + (end - start)); //time： 823</span></span>
<span class="line"><span>}</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>// 算法复杂度为 O(1)</span></span>
<span class="line"><span>@Test</span></span>
<span class="line"><span>public void test2(){</span></span>
<span class="line"><span>    HashSet&#x3C;Integer> set = new HashSet&#x3C;>(100000);</span></span>
<span class="line"><span>    for(int i = 0;i &#x3C; 100000;i++){</span></span>
<span class="line"><span>    	set.add(i + 1);</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>    long start = System.currentTimeMillis();</span></span>
<span class="line"><span>    for(int j = 1; j&#x3C;=100000;j++) {</span></span>
<span class="line"><span>        int temp = j;</span></span>
<span class="line"><span>        boolean contains = set.contains(temp);</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>    long end = System.currentTimeMillis();</span></span>
<span class="line"><span>    System.out.println("time： " + (end - start)); //time： 5</span></span>
<span class="line"><span>}</span></span></code></pre>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617163657697.png" alt="image-20220617163657697" tabindex="0" loading="lazy"><figcaption>image-20220617163657697</figcaption></figure>
...
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>mysql> show variables like '%adaptive_hash_index';</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><h3 id="_6-3-二叉搜索树" tabindex="-1"><a class="header-anchor" href="#_6-3-二叉搜索树"><span>6.3 二叉搜索树</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617164022728.png" alt="image-20220617164022728" tabindex="0" loading="lazy"><figcaption>image-20220617164022728</figcaption></figure>
<p>但是特殊情况，就是有时候二叉树的深度非常大，比如：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617164053134.png" alt="image-20220617164053134" tabindex="0" loading="lazy"><figcaption>image-20220617164053134</figcaption></figure>
<p>为了提高查询效率，就需要 减少磁盘IO数 。为了减少磁盘IO的次数，就需要尽量 降低树的高度 ，需要把 原来“瘦高”的树结构变的“矮胖”，树的每层的分叉越多越好。</p>
<h3 id="_6-4-avl树" tabindex="-1"><a class="header-anchor" href="#_6-4-avl树"><span>6.4 AVL树</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617165105005.png" alt="image-20220617165105005" tabindex="0" loading="lazy"><figcaption>image-20220617165105005</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617165124685.png" alt="image-20220617165124685" tabindex="0" loading="lazy"><figcaption>image-20220617165124685</figcaption></figure>
<p>你能看到此时树的高度降低了，当数据量 N 大的时候，以及树的分叉树 M 大的时候，M叉树的高度会远小于二叉树的高度 (M &gt; 2)。所以，我们需要把 `树从“瘦高” 变 “矮胖”。</p>
<h3 id="_6-5-b-tree" tabindex="-1"><a class="header-anchor" href="#_6-5-b-tree"><span>6.5 B-Tree</span></a></h3>
<p>B 树的英文是 Balance Tree，也就是 <code v-pre>多路平衡查找树</code>。简写为 B-Tree。它的高度远小于平衡二叉树的高度。</p>
<p>B 树的结构如下图所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617165937875.png" alt="image-20220617165937875" tabindex="0" loading="lazy"><figcaption>image-20220617165937875</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617170526488.png" alt="image-20220617170526488" tabindex="0" loading="lazy"><figcaption>image-20220617170526488</figcaption></figure>
<h3 id="_6-6-b-tree" tabindex="-1"><a class="header-anchor" href="#_6-6-b-tree"><span>6.6 B+Tree</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617170710329.png" alt="image-20220617170710329" tabindex="0" loading="lazy"><figcaption>image-20220617170710329</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617171106671.png" alt="image-20220617171106671" tabindex="0" loading="lazy"><figcaption>image-20220617171106671</figcaption></figure>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617171131747.png" alt="image-20220617171131747" tabindex="0" loading="lazy"><figcaption>image-20220617171131747</figcaption></figure>
...
<img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617175309115.png" alt="image-20220617175309115" style="float:left;" />
<h3 id="_6-7-r树" tabindex="-1"><a class="header-anchor" href="#_6-7-r树"><span>6.7 R树</span></a></h3>
...
<h3 id="_6-8-小结" tabindex="-1"><a class="header-anchor" href="#_6-8-小结"><span>6.8 小结</span></a></h3>
...
<h3 id="附录-算法的时间复杂度" tabindex="-1"><a class="header-anchor" href="#附录-算法的时间复杂度"><span>附录：算法的时间复杂度</span></a></h3>
<p>同一问题可用不同算法解决，而一个算法的质量优劣将影响到算法乃至程序的效率。算法分析的目的在 于选择合适算法和改进算法。</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617175516191.png" alt="image-20220617175516191" tabindex="0" loading="lazy"><figcaption>image-20220617175516191</figcaption></figure>
