<template><div><p><strong>首先看下一下自动配置的整个流程图</strong></p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/template/自动配置.jpg" alt="%E8%87%AA%E5%8A%A8%E9%85%8D%E7%BD%AE.jpg" tabindex="0" loading="lazy"><figcaption>%E8%87%AA%E5%8A%A8%E9%85%8D%E7%BD%AE.jpg</figcaption></figure>
<p>自从有了 SpringBoot 之后，咋们就起飞了！各种零配置开箱即用，而我们之所以开发起来能够这么爽，<strong>自动配置</strong>的功劳少不了，今天我们就一起来讨论一下 SpringBoot 自动配置原理。</p>
<blockquote>
<p>Spring Boot的自动配置是通过@EnableAutoConfiguration注解实现的。当该注解被标记在一个类上时，Spring Boot就会根据应用程序中所引入的依赖，自动配置应用程序所需的Bean、服务和其他组件。</p>
</blockquote>
<h2 id="_1-spring-boot自动配置的概念" tabindex="-1"><a class="header-anchor" href="#_1-spring-boot自动配置的概念" aria-hidden="true">#</a> 1. Spring Boot自动配置的概念</h2>
<blockquote>
<p>Spring Boot自动配置是指，在Spring Boot中，通过一些规则来自动配置应用程序所需的Bean、服务和其他组件。这种自动配置的方式可以大大减少开发人员的工作量，因为他们不需要手动配置每个组件，而只需要在应用程序中引入所需的模块即可。</p>
</blockquote>
<h2 id="_2-逐步分析" tabindex="-1"><a class="header-anchor" href="#_2-逐步分析" aria-hidden="true">#</a> 2. 逐步分析</h2>
<h3 id="_2-1-springbootapplication" tabindex="-1"><a class="header-anchor" href="#_2-1-springbootapplication" aria-hidden="true">#</a> 2.1 @SpringBootApplication</h3>
<p>一切的来自起源SpringBoot的启动类，我们发现main方法上面有个注解：<code v-pre>@SpringBootApplication</code></p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>leo<span class="token punctuation">.</span>demo02</span><span class="token punctuation">;</span>
 ...
 * <span class="token keyword">@date</span> 2023/8/10 11:22
 */</span>
<span class="token annotation punctuation">@SpringBootApplication</span>
<span class="token annotation punctuation">@MapperScan</span><span class="token punctuation">(</span><span class="token string">"com.leo.demo02.mapper"</span><span class="token punctuation">)</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>@SpringBootApplication</code> 标注在某个类上说明这个类是 SpringBoot 的主配置类， SpringBoot 就应该运行这个类的main方法来启动 SpringBoot 应用；它的本质是一个组合注解，我们点进去查看该类的元信息主要包含3个注解：</p>
...
<li><code v-pre>@SpringBootConfiguration</code>（里面就是@Configuration，标注当前类为配置类，其实只是做了一层封装改了个名字而已）</li>
<li><code v-pre>@EnableAutoConfiguration</code>（开启自动配置）</li>
<li><code v-pre>@ComponentScan</code>（包扫描）</li>
</ul>
<p>注：@Inherited是一个标识，用来修饰注解，如果一个类用上了@Inherited修饰的注解，那么其子类也会继承这个注解</p>
<p>我们下面逐一分析这3个注解作用</p>
<h3 id="_2-2-springbootconfiguration" tabindex="-1"><a class="header-anchor" href="#_2-2-springbootconfiguration" aria-hidden="true">#</a> 2.2  @SpringBootConfiguration</h3>
<p>我们继续点<code v-pre>@SpringBootConfiguration</code>进去查看源码如下：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Target</span><span class="token punctuation">(</span><span class="token punctuation">{</span><span class="token class-name">ElementType</span><span class="token punctuation">.</span><span class="token constant">TYPE</span><span class="token punctuation">}</span><span class="token punctuation">)</span>
<span class="token annotation punctuation">@Retention</span><span class="token punctuation">(</span><span class="token class-name">RetentionPolicy</span><span class="token punctuation">.</span><span class="token constant">RUNTIME</span><span class="token punctuation">)</span>
<span class="token annotation punctuation">@Documented</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>@Configuration</code>标注在某个类上，表示这是一个 springboot的<code v-pre>配置类</code>。可以向容器中注入组件。</p>
<h3 id="_2-3-componentscan" tabindex="-1"><a class="header-anchor" href="#_2-3-componentscan" aria-hidden="true">#</a> 2.3 @ComponentScan</h3>
<ul>
<li><code v-pre>@ComponentScan</code>：配置用于 Configuration 类的组件扫描指令。</li>
<li>提供与 <code v-pre>Spring XML</code> 的 <code v-pre>&lt;context:component-scan&gt;</code> 元素并行的支持。</li>
<li>可以 <code v-pre>basePackageClasses</code> 或<code v-pre>basePackages </code>来定义要扫描的特定包。 如果没有定义特定的包，将从声明该注解的类的<code v-pre>包开始扫描</code>。</li>
</ul>
<h3 id="_2-4-enableautoconfiguration" tabindex="-1"><a class="header-anchor" href="#_2-4-enableautoconfiguration" aria-hidden="true">#</a> 2.4 @EnableAutoConfiguration</h3>
<ul>
<li><code v-pre>@ComponentScan</code>：配置用于 Configuration 类的组件扫描指令。</li>
<li>提供与 <code v-pre>Spring XML</code> 的 <code v-pre>&lt;context:component-scan&gt;</code> 元素并行的支持。</li>
<li>可以 <code v-pre>basePackageClasses</code> 或<code v-pre>basePackages </code>来定义要扫描的特定包。 如果没有定义特定的包，将从声明该注解的类的<code v-pre>包开始扫描</code>。</li>
</ul>
<h2 id="_3-自动配置" tabindex="-1"><a class="header-anchor" href="#_3-自动配置" aria-hidden="true">#</a> 3. 自动配置</h2>
...
<span class="token annotation punctuation">@Documented</span>
<span class="token annotation punctuation">@Inherited</span>
<span class="token annotation punctuation">@AutoConfigurationPackage</span>   <span class="token comment">//自动导包</span>
<span class="token annotation punctuation">@Import</span><span class="token punctuation">(</span><span class="token punctuation">{</span><span class="token class-name">AutoConfigurationImportSelector</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">}</span><span class="token punctuation">)</span> <span class="token comment">//自动配置导入选择</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3-1-autoconfigurationpackage" tabindex="-1"><a class="header-anchor" href="#_3-1-autoconfigurationpackage" aria-hidden="true">#</a> 3.1 @AutoConfigurationPackage</h3>
<ul>
<li>自动导入配置包</li>
<li>点进去查看代码：</li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>@Import</code> 为spring的注解，导入一个配置文件，在springboot中为给容器导入一个组件，而导入的组件由 AutoConfigurationPackages.class的内部类<code v-pre>Registrar.class</code> 执行逻辑来决定是如何导入的。</p>
<p><strong>@Import({Registrar.class})</strong></p>
<p>点Registrar.class进去查看源码如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>注：Registrar实现了<code v-pre>ImportBeanDefinitionRegistrar</code>类，就可以被注解@Import导入到spring容器里。</p>
<p>这个地方打断点</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/cisyam/202308132036006.png" alt="image-20230813203640545" tabindex="0" loading="lazy"><figcaption>image-20230813203640545</figcaption></figure>
<p>运行可以查看到<code v-pre>(String[])(new AutoConfigurationPackages.PackageImports(metadata)).getPackageNames().toArray(new String[0])</code>的值为<code v-pre>com.leo.Applicaiton</code>：当前启动类所在的包名</p>
<p>结论： <strong>@AutoConfigurationPackage 就是将主配置类（@SpringBootApplication 标注的类）所在的包下面所有的组件都扫描注册到 spring 容器中。</strong></p>
<h3 id="_3-2-import-autoconfigurationimportselector-class" tabindex="-1"><a class="header-anchor" href="#_3-2-import-autoconfigurationimportselector-class" aria-hidden="true">#</a> 3.2  @Import({AutoConfigurationImportSelector.class})</h3>
<p>作用：AutoConfigurationImportSelector<code v-pre>开启自动配置类的导包的选择器</code>，即是带入哪些类，有选择性的导入</p>
<p>点AutoConfigurationImportSelector.class进入查看源码，这个类中有两个方法见名知意：</p>
<ol>
<li>
<p>selectImports：选择需要导入的组件</p>
...
<li>
<p>getAutoConfigurationEntry：根据导入的@Configuration类的AnnotationMetadata返回AutoConfigurationImportSelector.AutoConfigurationEntry</p>
</li>
</ol>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">protected</span> <span class="token class-name">AutoConfigurationImportSelector<span class="token punctuation">.</span>AutoConfigurationEntry</span> <span class="token function">getAutoConfigurationEntry</span><span class="token punctuation">(</span><span class="token class-name">AnnotationMetadata</span> annotationMetadata<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span> <span class="token keyword">else</span> <span class="token punctuation">{</span>
        <span class="token class-name">AnnotationAttributes</span> attributes <span class="token operator">=</span> <span class="token keyword">this</span><span class="token punctuation">.</span><span class="token function">getAttributes</span><span class="token punctuation">(</span>annotationMetadata<span class="token punctuation">)</span><span class="token punctuation">;</span>
         <span class="token comment">// 这打个断点，看看 返回的数据</span>
        ...
        <span class="token comment">//删除重复项</span>
        ...
        <span class="token comment">//删除需要排除的依赖</span>
...
<p>this.getCandidateConfigurations(annotationMetadata, attributes)这里断点查看</p>
</blockquote>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/cisyam/202308132043891.png" alt="image-20230813204333506" tabindex="0" loading="lazy"><figcaption>image-20230813204333506</figcaption></figure>
<p>configurations数组长度为127，并且文件后缀名都为 <code v-pre>**AutoConfiguration</code></p>
<p>结论： <strong>这些都是候选的配置类，经过去重，去除需要的排除的依赖，最终的组件才是这个环境需要的所有组件。有了自动配置，就不需要我们自己手写配置的值了，配置类有默认值的。</strong></p>
<p>我们继续往下看看是如何返回需要配置的组件的</p>
<h4 id="_1-getcandidateconfigurations-annotationmetadata-attributes" tabindex="-1"><a class="header-anchor" href="#_1-getcandidateconfigurations-annotationmetadata-attributes" aria-hidden="true">#</a> 1. getCandidateConfigurations(annotationMetadata, attributes)</h4>
<p>方法如下：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/cisyam/202308132054544.png" alt="image-20230813205426429" tabindex="0" loading="lazy"><figcaption>image-20230813205426429</figcaption></figure>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里有句断言： Assert.notEmpty(configurations, &quot;No auto configuration classes found in META-INF/spring.factories. If you are using a custom packaging, make sure that file is correct.&quot;);</p>
<p>意思是：“在 META-INF/spring.factories 中没有找到自动配置类。如果您使用自定义包装，请确保该文件是正确的。“</p>
<p>结论： <strong>即是要loadFactoryNames（）方法要找到自动的配置类返回才不会报错。</strong></p>
<h4 id="_2-getspringfactoriesloaderfactoryclass" tabindex="-1"><a class="header-anchor" href="#_2-getspringfactoriesloaderfactoryclass" aria-hidden="true">#</a> 2.  getSpringFactoriesLoaderFactoryClass()</h4>
<p>我们点进去发现：this.getSpringFactoriesLoaderFactoryClass()返回的是<code v-pre>EnableAutoConfiguration.class</code>这个注解。这个注解和@SpringBootApplication下标识注解是同一个注解。</p>
...
<span class="token keyword">protected</span> <span class="token class-name">Class</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token operator">?</span><span class="token punctuation">></span></span> <span class="token function">getSpringFactoriesLoaderFactoryClass</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token keyword">return</span> <span class="token class-name">EnableAutoConfiguration</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>结论：<strong>获取一个能加载自动配置类的类，即SpringBoot默认自动配置类为EnableAutoConfiguration</strong></p>
<h5 id="loadfactorynames" tabindex="-1"><a class="header-anchor" href="#loadfactorynames" aria-hidden="true">#</a> loadFactoryNames()</h5>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/cisyam/202308132057526.png" alt="image-20230813205719355" tabindex="0" loading="lazy"><figcaption>image-20230813205719355</figcaption></figure>
<p>先是将 <code v-pre>EnableAutoConfiguration.class</code> 传给了 <code v-pre>factoryType</code></p>
<p>然后<code v-pre>String factoryTypeName = factoryType.getName();</code>，所以<code v-pre>factoryTypeName</code> 值为 <code v-pre> org.springframework.boot.autoconfigure.EnableAutoConfiguration</code></p>
<h5 id="loadspringfactories" tabindex="-1"><a class="header-anchor" href="#loadspringfactories" aria-hidden="true">#</a> loadSpringFactories()</h5>
<p>接着查看loadSpringFactories方法的作用</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">private</span> <span class="token keyword">static</span> <span class="token class-name">Map</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">String</span><span class="token punctuation">,</span> <span class="token class-name">List</span><span class="token punctuation">&lt;</span><span class="token class-name">String</span><span class="token punctuation">></span><span class="token punctuation">></span></span> <span class="token function">loadSpringFactories</span><span class="token punctuation">(</span><span class="token class-name">ClassLoader</span> classLoader<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//断点查看</span>
   ...
   <span class="token keyword">try</span> <span class="token punctuation">{</span>
      <span class="token comment">//注意这里：META-INF/spring.factories</span>
      ...
      <span class="token comment">//去重，断点查看result值</span>
   ...
   <span class="token keyword">return</span> result<span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>这里的 FACTORIES_RESOURCE_LOCATION 在上面有定义：META-INF/spring.factories</strong></p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">final</span> <span class="token keyword">class</span> <span class="token class-name">SpringFactoriesLoader</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>META-INF/spring.factories文件在哪里？？ 在所有引入的java包的当前类路径下的META-INF/spring.factories文件都会被读取，如：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/cisyam/202308132102739.png" alt="image-20230813210249346" tabindex="0" loading="lazy"><figcaption>image-20230813210249346</figcaption></figure>
<blockquote>
<p>该方法作用是加载所有依赖的路径META-INF/spring.factories文件，通过map结构保存，key为文件中定义的一些标识工厂类，value就是能自动配置的一些工厂实现的类，value用list保存并去重。</p>
</blockquote>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/cisyam/202308132104637.png" alt="image-20230813210401399" tabindex="0" loading="lazy"><figcaption>image-20230813210401399</figcaption></figure>
<p>在回看 <code v-pre>loadSpringFactories(classLoaderToUse).getOrDefault(factoryTypeName, Collections.emptyList());</code></p>
<p>因为 <code v-pre>loadFactoryNames</code> 方法携带过来的第一个参数为 <code v-pre>EnableAutoConfiguration.class</code>，所以 <code v-pre>factoryType</code> 值也为 <code v-pre>EnableAutoConfiguration.class</code>，那么 <code v-pre>factoryTypeName</code> 值为 <code v-pre>EnableAutoConfiguration</code>。拿到的值就是META-INF/spring.factories文件下的key为 org.springframework.boot.autoconfigure.EnableAutoConfiguration的值</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/cisyam/202308132118638.png" alt="image-20230813211838324" tabindex="0" loading="lazy"><figcaption>image-20230813211838324</figcaption></figure>
<p><code v-pre>getOrDefault</code> 当 <code v-pre>Map</code> 集合中有这个 key 时，就使用这个 key值，如果没有就使用默认值空数组</p>
<p>结论：</p>
<ul>
<li><strong>loadSpringFactories()该方法就是从“META-INF/spring.factories”中加载给定类型的工厂实现的完全限定类名放到map中</strong></li>
<li><strong>loadFactoryNames()是根据SpringBoot的启动生命流程，当需要加载自动配置类时，就会传入org.springframework.boot.autoconfigure.EnableAutoConfiguration参数，从map中查找key为org.springframework.boot.autoconfigure.EnableAutoConfiguration的值，这些值通过反射加到容器中，之后的作用就是用它们来做自动配置，这就是Springboot自动配置开始的地方</strong></li>
<li><strong>只有这些自动配置类进入到容器中以后，接下来这个自动配置类才开始进行启动</strong></li>
<li>当需要其他的配置时如监听相关配置：listenter，就传不同的参数，获取相关的listenter配置。</li>
</ul>
<h2 id="_4-自动配置流程细节梳理" tabindex="-1"><a class="header-anchor" href="#_4-自动配置流程细节梳理" aria-hidden="true">#</a> 4. 自动配置流程细节梳理</h2>
<p>导入<code v-pre>starter-web</code>：导入了web开发场景</p>
<ul>
<li>1、场景启动器导入了相关场景的所有依赖：<code v-pre>starter-json</code>、<code v-pre>starter-tomcat</code>、<code v-pre>springmvc</code></li>
<li>2、每个场景启动器都引入了一个<code v-pre>spring-boot-starter</code>，核心场景启动器。</li>
<li>3、<strong>核心场景启动器</strong>引入了<code v-pre>spring-boot-autoconfigure</code>包。</li>
<li>4、<code v-pre>spring-boot-autoconfigure</code>里面囊括了所有场景的所有配置。</li>
<li>5、只要这个包下的所有类都能生效，那么相当于SpringBoot官方写好的整合功能就生效了。</li>
<li>6、SpringBoot默认却扫描不到 <code v-pre>spring-boot-autoconfigure</code>下写好的所有<strong>配置类</strong>。（这些<strong>配置类</strong>给我们做了整合操作），<strong>默认只扫描主程序所在的包</strong>。</li>
</ul>
<p>2、<strong>主程序</strong>：<code v-pre>@SpringBootApplication</code></p>
<ul>
<li>
<p><code v-pre>@SpringBootApplication</code>由三个注解组成<code v-pre>@SpringBootConfiguration</code>、<code v-pre>@EnableAutoConfiguratio</code>、<code v-pre>@ComponentScan</code></p>
</li>
<li>
<p>SpringBoot默认只能扫描自己主程序所在的包及其下面的子包，扫描不到 <code v-pre>spring-boot-autoconfigure</code>包中官方写好的<strong>配置类</strong></p>
</li>
<li>
<p><code v-pre>**@EnableAutoConfiguration**</code>：SpringBoot <strong>开启自动配置的核心</strong>。</p>
</li>
<li>
<ul>
<li>是由<code v-pre>@Import(AutoConfigurationImportSelector.class)</code>提供功能：批量给容器中导入组件。</li>
<li>SpringBoot启动会默认加载 142个配置类。</li>
<li>这<strong>142个配置类</strong>来自于<code v-pre>spring-boot-autoconfigure</code>下 <code v-pre>META-INF/spring/**org.springframework.boot.autoconfigure.AutoConfiguration**.imports</code>文件指定的</li>
<li>项目启动的时候利用 @Import 批量导入组件机制把 <code v-pre>autoconfigure</code> 包下的142 <code v-pre>xxxxAutoConfiguration</code>类导入进来（<strong>自动配置类</strong>）</li>
<li>虽然导入了<code v-pre>142</code>个自动配置类</li>
</ul>
</li>
<li>
<p>按需生效：</p>
...
<li>
<ul>
<li>并不是这<code v-pre>142</code>个自动配置类都能生效</li>
<li>每一个自动配置类，都有条件注解<code v-pre>@ConditionalOnxxx</code>，只有条件成立，才能生效</li>
</ul>
</li>
</ul>
<p><strong>3、</strong><code v-pre>**xxxxAutoConfiguration**</code><strong>自动配置类</strong></p>
<ul>
<li><strong>给容器中使用@Bean 放一堆组件。</strong></li>
<li>每个<strong>自动配置类</strong>都可能有这个注解<code v-pre>@EnableConfigurationProperties(**ServerProperties**.class)</code>，用来把配置文件中配的指定前缀的属性值封装到 <code v-pre>xxxProperties</code><strong>属性类</strong>中</li>
<li>以Tomcat为例：把服务器的所有配置都是以<code v-pre>server</code>开头的。配置都封装到了属性类中。</li>
<li>给<strong>容器</strong>中放的所有<strong>组件</strong>的一些<strong>核心参数</strong>，都来自于<code v-pre>**xxxProperties**</code><strong>。</strong><code v-pre>**xxxProperties**</code><strong>都是和配置文件绑定。</strong></li>
<li><strong>只需要改配置文件的值，核心组件的底层参数都能修改</strong></li>
</ul>
<p>**4、**写业务，全程无需关心各种整合（底层这些整合写好了，而且也生效了）</p>
<p><strong>核心流程总结：</strong></p>
<p>1、导入<code v-pre>starter</code>，就会导入<code v-pre>autoconfigure</code>包。</p>
<p>2、<code v-pre>autoconfigure</code> 包里面 有一个文件 <code v-pre>META-INF/spring/**org.springframework.boot.autoconfigure.AutoConfiguration**.imports</code>,里面指定的所有启动要加载的自动配置类</p>
<p>3、@EnableAutoConfiguration 会自动的把上面文件里面写的所有<strong>自动配置类都导入进来。xxxAutoConfiguration 是有条件注解进行按需加载</strong></p>
<p>4、<code v-pre>xxxAutoConfiguration</code>给容器中导入一堆组件，组件都是从 <code v-pre>xxxProperties</code>中提取属性值</p>
<p>5、<code v-pre>xxxProperties</code>又是和<strong>配置文件</strong>进行了绑定</p>
<p>**效果：**导入<code v-pre>starter</code>、修改配置文件，就能修改底层行为。</p>
<h2 id="_5-常用的conditional注解" tabindex="-1"><a class="header-anchor" href="#_5-常用的conditional注解" aria-hidden="true">#</a> 5. 常用的Conditional注解</h2>
<ul>
<li>在加载自动配置类的时候，并不是将spring.factories的配置全部加载进来，而是通过@Conditional等注解的判断进行动态加载</li>
<li>@Conditional其实是spring底层注解，意思就是根据不同的条件，来进行自己不同的条件判断，如果满足指定的条件，那么配置类里边的配置才会生效。</li>
<li>常用的Conditional注解：
<ul>
<li>@ConditionalOnClass ： classpath中存在该类时起效</li>
<li>@ConditionalOnMissingClass ： classpath中不存在该类时起效</li>
<li>@ConditionalOnBean ： DI容器中存在该类型Bean时起效</li>
<li>@ConditionalOnMissingBean ： DI容器中不存在该类型Bean时起效</li>
<li>@ConditionalOnSingleCandidate ： DI容器中该类型Bean只有一个或@Primary的只有一个时起效</li>
<li>@ConditionalOnExpression ： SpEL表达式结果为true时</li>
<li>@ConditionalOnProperty ： 参数设置或者值一致时起效</li>
<li>@ConditionalOnResource ： 指定的文件存在时起效</li>
<li>@ConditionalOnJndi ： 指定的JNDI存在时起效</li>
<li>@ConditionalOnJava ： 指定的Java版本存在时起效</li>
<li>@ConditionalOnWebApplication ： Web应用环境下起效</li>
<li>@ConditionalOnNotWebApplication ： 非Web应用环境下起效</li>
</ul>
</li>
</ul>
