...
<h2 id="_1-什么是mvcc" tabindex="-1"><a class="header-anchor" href="#_1-什么是mvcc"><span>1. 什么是MVCC</span></a></h2>
...
<h2 id="_2-快照读与当前读" tabindex="-1"><a class="header-anchor" href="#_2-快照读与当前读"><span>2. 快照读与当前读</span></a></h2>
...
<h3 id="_2-1-快照读" tabindex="-1"><a class="header-anchor" href="#_2-1-快照读"><span>2.1 快照读</span></a></h3>
<p>快照读又叫一致性读，读取的是快照数据。<strong>不加锁的简单的 SELECT 都属于快照读</strong>，即不加锁的非阻塞 读；比如这样：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>SELECT * FROM player WHERE ...</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>之所以出现快照读的情况，是基于提高并发性能的考虑，快照读的实现是基于MVCC，它在很多情况下， 避免了加锁操作，降低了开销。</p>
<p>既然是基于多版本，那么快照读可能读到的并不一定是数据的最新版本，而有可能是之前的历史版本。</p>
<p>快照读的前提是隔离级别不是串行级别，串行级别下的快照读会退化成当前读。</p>
<h3 id="_2-2-当前读" tabindex="-1"><a class="header-anchor" href="#_2-2-当前读"><span>2.2 当前读</span></a></h3>
<p>当前读读取的是记录的最新版本（最新数据，而不是历史版本的数据），读取时还要保证其他并发事务 不能修改当前记录，会对读取的记录进行加锁。加锁的 SELECT，或者对数据进行增删改都会进行当前 读。比如：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>SELECT * FROM student LOCK IN SHARE MODE; # 共享锁</span></span>
<span class="line"><span>SELECT * FROM student FOR UPDATE; # 排他锁</span></span>
<span class="line"><span>INSERT INTO student values ... # 排他锁</span></span>
<span class="line"><span>DELETE FROM student WHERE ... # 排他锁</span></span>
<span class="line"><span>UPDATE student SET ... # 排他锁</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_3-复习" tabindex="-1"><a class="header-anchor" href="#_3-复习"><span>3. 复习</span></a></h2>
<h3 id="_3-1-再谈隔离级别" tabindex="-1"><a class="header-anchor" href="#_3-1-再谈隔离级别"><span>3.1 再谈隔离级别</span></a></h3>
<p>我们知道事务有 4 个隔离级别，可能存在三种并发问题：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220714140441064.png" alt="image-20220714140441064" tabindex="0" loading="lazy"><figcaption>image-20220714140441064</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220714140541555.png" alt="image-20220714140541555" tabindex="0" loading="lazy"><figcaption>image-20220714140541555</figcaption></figure>
<h3 id="_3-2-隐藏字段、undo-log版本链" tabindex="-1"><a class="header-anchor" href="#_3-2-隐藏字段、undo-log版本链"><span>3.2 隐藏字段、Undo Log版本链</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220714140801595.png" alt="image-20220714140801595" tabindex="0" loading="lazy"><figcaption>image-20220714140801595</figcaption></figure>
<blockquote>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220714140846658.png" alt="image-20220714140846658" tabindex="0" loading="lazy"><figcaption>image-20220714140846658</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220714141012874.png" alt="image-20220714141012874" tabindex="0" loading="lazy"><figcaption>image-20220714141012874</figcaption></figure>
...
<h2 id="_4-mvcc实现原理之readview" tabindex="-1"><a class="header-anchor" href="#_4-mvcc实现原理之readview"><span>4. MVCC实现原理之ReadView</span></a></h2>
<p>MVCC 的实现依赖于：<code v-pre>隐藏字段</code>、<code v-pre>Undo Log</code>、<code v-pre>Read View</code>。</p>
<h3 id="_4-1-什么是readview" tabindex="-1"><a class="header-anchor" href="#_4-1-什么是readview"><span>4.1 什么是ReadView</span></a></h3>
...
<h3 id="_4-2-设计思路" tabindex="-1"><a class="header-anchor" href="#_4-2-设计思路"><span>4.2 设计思路</span></a></h3>
...
<h3 id="_4-3-readview的规则" tabindex="-1"><a class="header-anchor" href="#_4-3-readview的规则"><span>4.3 ReadView的规则</span></a></h3>
...
<h3 id="_4-4-mvcc整体操作流程" tabindex="-1"><a class="header-anchor" href="#_4-4-mvcc整体操作流程"><span>4.4 MVCC整体操作流程</span></a></h3>
...
<p>如表所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715130843147.png" alt="image-20220715130843147" tabindex="0" loading="lazy"><figcaption>image-20220715130843147</figcaption></figure>
<blockquote>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715130916437.png" alt="image-20220715130916437" tabindex="0" loading="lazy"><figcaption>image-20220715130916437</figcaption></figure>
<h2 id="_5-举例说明" tabindex="-1"><a class="header-anchor" href="#_5-举例说明"><span>5. 举例说明</span></a></h2>
...
<h3 id="_5-1-read-committed隔离级别下" tabindex="-1"><a class="header-anchor" href="#_5-1-read-committed隔离级别下"><span>5.1 READ COMMITTED隔离级别下</span></a></h3>
...
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span># Transaction 10</span></span>
<span class="line"><span>BEGIN;</span></span>
<span class="line"><span>UPDATE student SET name="李四" WHERE id=1;</span></span>
<span class="line"><span>UPDATE student SET name="王五" WHERE id=1;</span></span>
<span class="line"><span># Transaction 20</span></span>
<span class="line"><span>BEGIN;</span></span>
<span class="line"><span># 更新了一些别的表的记录</span></span>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715133640655.png" alt="image-20220715133640655" tabindex="0" loading="lazy"><figcaption>image-20220715133640655</figcaption></figure>
<p>假设现在有一个使用 <code v-pre>READ COMMITTED</code> 隔离级别的事务开始执行：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span># 使用READ COMMITTED隔离级别的事务</span></span>
<span class="line"><span>BEGIN;</span></span>
<span class="line"><span></span></span>
<span class="line"><span># SELECT1：Transaction 10、20未提交</span></span>
<span class="line"><span>SELECT * FROM student WHERE id = 1; # 得到的列name的值为'张三'</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715134540737.png" alt="image-20220715134540737" style="float:left;" />
...
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span># Transaction 20</span></span>
<span class="line"><span>BEGIN;</span></span>
<span class="line"><span># 更新了一些别的表的记录</span></span>
<span class="line"><span>...</span></span>
<span class="line"><span>UPDATE student SET name="钱七" WHERE id=1;</span></span>
<span class="line"><span>UPDATE student SET name="宋八" WHERE id=1;</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>此刻，表student中 <code v-pre>id</code> 为 <code v-pre>1</code> 的记录的版本链就长这样：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715134839081.png" alt="image-20220715134839081" tabindex="0" loading="lazy"><figcaption>image-20220715134839081</figcaption></figure>
<p>然后再到刚才使用 <code v-pre>READ COMMITTED</code> 隔离级别的事务中继续查找这个 id 为 1 的记录，如下：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span># 使用READ COMMITTED隔离级别的事务</span></span>
<span class="line"><span>BEGIN;</span></span>
<span class="line"><span></span></span>
<span class="line"><span># SELECT1：Transaction 10、20均未提交</span></span>
<span class="line"><span>SELECT * FROM student WHERE id = 1; # 得到的列name的值为'张三'</span></span>
<span class="line"><span></span></span>
<span class="line"><span># SELECT2：Transaction 10提交，Transaction 20未提交</span></span>
<span class="line"><span>SELECT * FROM student WHERE id = 1; # 得到的列name的值为'王五'</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715135017000.png" alt="image-20220715135017000" style="float:left;" />
...
<h3 id="_5-2-repeatable-read隔离级别下" tabindex="-1"><a class="header-anchor" href="#_5-2-repeatable-read隔离级别下"><span>5.2 REPEATABLE READ隔离级别下</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715140006061.png" alt="image-20220715140006061" tabindex="0" loading="lazy"><figcaption>image-20220715140006061</figcaption></figure>
<p>假设现在有一个使用 <code v-pre>REPEATABLE READ</code> 隔离级别的事务开始执行：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span># 使用REPEATABLE READ隔离级别的事务</span></span>
<span class="line"><span>BEGIN;</span></span>
<span class="line"><span></span></span>
<span class="line"><span># SELECT1：Transaction 10、20未提交</span></span>
<span class="line"><span>SELECT * FROM student WHERE id = 1; # 得到的列name的值为'张三'</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715140155744.png" alt="image-20220715140155744" style="float:left;" />
...
<span class="line"><span>UPDATE student SET name="钱七" WHERE id=1;</span></span>
<span class="line"><span>UPDATE student SET name="宋八" WHERE id=1;</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>此刻，表student 中 <code v-pre>id</code> 为 <code v-pre>1</code> 的记录的版本链长这样：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715140354217.png" alt="image-20220715140354217" tabindex="0" loading="lazy"><figcaption>image-20220715140354217</figcaption></figure>
...
<span class="line"><span>SELECT * FROM student WHERE id = 1; # 得到的列name的值为'张三'</span></span>
<span class="line"><span># SELECT2：Transaction 10提交，Transaction 20未提交</span></span>
<span class="line"><span>SELECT * FROM student WHERE id = 1; # 得到的列name的值仍为'张三'</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715140555172.png" alt="image-20220715140555172" style="float:left;" />
...
<h3 id="_5-3-如何解决幻读" tabindex="-1"><a class="header-anchor" href="#_5-3-如何解决幻读"><span>5.3 如何解决幻读</span></a></h3>
...
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>select * from student where id >= 1;</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>在开始查询之前，MySQL 会为事务 A 产生一个 ReadView，此时 ReadView 的内容如下： <code v-pre>trx_ids= [20,30] ， up_limit_id=20 ， low_limit_id=31 ， creator_trx_id=20</code> 。</p>
...
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>insert into student(id,name) values(2,'李四');</span></span>
<span class="line"><span>insert into student(id,name) values(3,'王五');</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>此时表student 中就有三条数据了，对应的 undo 如下图所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715141208667.png" alt="image-20220715141208667" tabindex="0" loading="lazy"><figcaption>image-20220715141208667</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220715141243993.png" alt="image-20220715141243993" tabindex="0" loading="lazy"><figcaption>image-20220715141243993</figcaption></figure>
