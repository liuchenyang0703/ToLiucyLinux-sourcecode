...
<h2 id="_1-数据库的存储结构-页" tabindex="-1"><a class="header-anchor" href="#_1-数据库的存储结构-页"><span>1. 数据库的存储结构：页</span></a></h2>
...
<h3 id="_1-1-磁盘与内存交互基本单位-页" tabindex="-1"><a class="header-anchor" href="#_1-1-磁盘与内存交互基本单位-页"><span>1.1 磁盘与内存交互基本单位：页</span></a></h3>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617193939742.png" alt="image-20220617193939742" tabindex="0" loading="lazy"><figcaption>image-20220617193939742</figcaption></figure>
<h3 id="_1-2-页结构概述" tabindex="-1"><a class="header-anchor" href="#_1-2-页结构概述"><span>1.2 页结构概述</span></a></h3>
...
<h3 id="_1-3-页的大小" tabindex="-1"><a class="header-anchor" href="#_1-3-页的大小"><span>1.3 页的大小</span></a></h3>
<p>不同的数据库管理系统（简称DBMS）的页大小不同。比如在 MySQL 的 InnoDB 存储引擎中，默认页的大小是 <code v-pre>16KB</code>，我们可以通过下面的命令来进行查看：</p>
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>show variables like '%innodb_page_size%'</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>SQL Server 中页的大小为 <code v-pre>8KB</code>，而在 Oracle 中我们用术语 &quot;<code v-pre>块</code>&quot; （Block）来表示 &quot;页&quot;，Oracle 支持的快大小为2KB, 4KB, 8KB, 16KB, 32KB 和 64KB。</p>
<h3 id="_1-4-页的上层结构" tabindex="-1"><a class="header-anchor" href="#_1-4-页的上层结构"><span>1.4 页的上层结构</span></a></h3>
<p>另外在数据库中，还存在着区（Extent）、段（Segment）和表空间（Tablespace）的概念。行、页、区、段、表空间的关系如下图所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617194256988.png" alt="image-20220617194256988" tabindex="0" loading="lazy"><figcaption>image-20220617194256988</figcaption></figure>
...
<h2 id="_2-页的内部结构" tabindex="-1"><a class="header-anchor" href="#_2-页的内部结构"><span>2. 页的内部结构</span></a></h2>
<p>页如果按类型划分的话，常见的有 <code v-pre>数据页（保存B+树节点）、系统表、Undo 页 和 事务数据页</code> 等。数据页是我们最常使用的页。</p>
...
<p>页结构的示意图如下所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617195012446.png" alt="image-20220617195012446" tabindex="0" loading="lazy"><figcaption>image-20220617195012446</figcaption></figure>
<p>如下表所示：</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220617195148164.png" alt="image-20220617195148164" tabindex="0" loading="lazy"><figcaption>image-20220617195148164</figcaption></figure>
<p>我们可以把这7个结构分为3个部分。</p>
<h3 id="第一部分-file-header-文件头部-和-file-trailer-文件尾部" tabindex="-1"><a class="header-anchor" href="#第一部分-file-header-文件头部-和-file-trailer-文件尾部"><span>第一部分：File Header (文件头部) 和 File Trailer (文件尾部)</span></a></h3>
<p>见文件InnoDB数据库存储结构.mmap</p>
<h3 id="第二部分-user-records-用户记录-、最大最小记录、free-space-空闲空间" tabindex="-1"><a class="header-anchor" href="#第二部分-user-records-用户记录-、最大最小记录、free-space-空闲空间"><span>第二部分：User Records (用户记录)、最大最小记录、Free Space (空闲空间)</span></a></h3>
<p>见文件InnoDB数据库存储结构.mmap</p>
<h3 id="第三部分-page-directory-页目录-和-page-header-页面头部" tabindex="-1"><a class="header-anchor" href="#第三部分-page-directory-页目录-和-page-header-页面头部"><span>第三部分：Page Directory (页目录) 和 Page Header (页面头部)</span></a></h3>
<p>见文件InnoDB数据库存储结构.mmap</p>
<h3 id="_2-3-从数据库页的角度看b-树如何查询" tabindex="-1"><a class="header-anchor" href="#_2-3-从数据库页的角度看b-树如何查询"><span>2.3 从数据库页的角度看B+树如何查询</span></a></h3>
<p>一颗B+树按照字节类型可以分为两部分：</p>
<ol>
<li>叶子节点，B+ 树最底层的节点，节点的高度为0，存储行记录。</li>
<li>非叶子节点，节点的高度大于0，存储索引键和页面指针，并不存储行记录本身。</li>
</ol>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220620221112635.png" alt="image-20220620221112635" tabindex="0" loading="lazy"><figcaption>image-20220620221112635</figcaption></figure>
...
<h2 id="_3-innodb行格式-或记录格式" tabindex="-1"><a class="header-anchor" href="#_3-innodb行格式-或记录格式"><span>3. InnoDB行格式 (或记录格式)</span></a></h2>
<p>见文件InnoDB数据库存储结构.mmap</p>
<h2 id="_4-区、段与碎片区" tabindex="-1"><a class="header-anchor" href="#_4-区、段与碎片区"><span>4. 区、段与碎片区</span></a></h2>
<h3 id="_4-1-为什么要有区" tabindex="-1"><a class="header-anchor" href="#_4-1-为什么要有区"><span>4.1 为什么要有区？</span></a></h3>
...
<h3 id="_4-2-为什么要有段" tabindex="-1"><a class="header-anchor" href="#_4-2-为什么要有段"><span>4.2 为什么要有段？</span></a></h3>
...
<h3 id="_4-3-为什么要有碎片区" tabindex="-1"><a class="header-anchor" href="#_4-3-为什么要有碎片区"><span>4.3 为什么要有碎片区？</span></a></h3>
...
<h3 id="_4-4-区的分类" tabindex="-1"><a class="header-anchor" href="#_4-4-区的分类"><span>4.4 区的分类</span></a></h3>
<p>区大体上可以分为4种类型：</p>
<ul>
...
<h2 id="_5-表空间" tabindex="-1"><a class="header-anchor" href="#_5-表空间"><span>5. 表空间</span></a></h2>
...
<h3 id="_5-1-独立表空间" tabindex="-1"><a class="header-anchor" href="#_5-1-独立表空间"><span>5.1 独立表空间</span></a></h3>
<p>独立表空间，即每张表有一个独立的表空间，也就是数据和索引信息都会保存在自己的表空间中。独立的表空间 (即：单表) 可以在不同的数据库之间进行 <code v-pre>迁移</code>。</p>
...
<div class="language-mysql line-numbers-mode" data-highlighter="shiki" data-ext="mysql" data-title="mysql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>show variables like 'innodb_file_per_table'</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>你能看到 innodb_file_per_table=ON, 这就意味着每张表都会单词保存一个 .ibd 文件。</p>
<h3 id="_5-2-系统表空间" tabindex="-1"><a class="header-anchor" href="#_5-2-系统表空间"><span>5.2 系统表空间</span></a></h3>
...
<p>这些系统表也称为 <code v-pre>数据字典</code>，它们都是以 B+ 树的形式保存在系统表空间的某个页面中。其中 <code v-pre>SYS_TABLES、SYS_COLUMNS、SYS_INDEXES、SYS_FIELDS</code> 这四个表尤其重要，称之为基本系统表 (basic system tables) ，我们先看看这4个表的结构：</p>
...
<h2 id="附录-数据页加载的三种方式" tabindex="-1"><a class="header-anchor" href="#附录-数据页加载的三种方式"><span>附录：数据页加载的三种方式</span></a></h2>
<p>InnoDB从磁盘中读取数据 <code v-pre>最小单位</code> 是数据页。而你想得到的 id = xxx 的数据，就是这个数据页众多行中的一行。</p>
<p>对于MySQL存放的数据，逻辑概念上我们称之为表，在磁盘等物理层面而言是按 <code v-pre>数据页</code> 形式进行存放的，当其加载到 MySQL 中我们称之为 <code v-pre>缓存页</code>。</p>
<p>如果缓冲池没有该页数据，那么缓冲池有以下三种读取数据的方式，每种方式的读取速率是不同的：</p>
<p><strong>1. 内存读取</strong></p>
<p>如果该数据存在于内存中，基本上执行时间在 1ms 左右，效率还是很高的。</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220621135638283.png" alt="image-20220621135638283" tabindex="0" loading="lazy"><figcaption>image-20220621135638283</figcaption></figure>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/img/image-20220621135737422.png" alt="image-20220621135737422" tabindex="0" loading="lazy"><figcaption>image-20220621135737422</figcaption></figure>
