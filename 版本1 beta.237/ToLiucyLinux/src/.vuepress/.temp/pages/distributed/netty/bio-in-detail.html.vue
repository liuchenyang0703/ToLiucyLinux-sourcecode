<template><div><h1 id="netty实战专栏-bio详解" tabindex="-1"><a class="header-anchor" href="#netty实战专栏-bio详解" aria-hidden="true">#</a> Netty实战专栏 | BIO详解</h1>
<h2 id="_1-前言" tabindex="-1"><a class="header-anchor" href="#_1-前言" aria-hidden="true">#</a> 1.前言</h2>
<p>大家好，我是Leo哥🫣🫣🫣，上一篇博客我们主要大致了解了一下Java中IO的演变历程，大体分为基础IO--&gt;BIO--&gt;NIO--&gt;AIO这几个阶段，分部逐层递进。下面几个章节我们就要来聊聊这个变化。本节主要讨论关于BIO的知识。好了，话不多说让我们开始吧😎😎😎。</p>
<h2 id="_2-前置知识" tabindex="-1"><a class="header-anchor" href="#_2-前置知识" aria-hidden="true">#</a> 2.前置知识</h2>
<p>在学习BIO之前，我们先了解一下IO相关的前置知识。</p>
<h3 id="_2-1阻塞io和非阻塞io" tabindex="-1"><a class="header-anchor" href="#_2-1阻塞io和非阻塞io" aria-hidden="true">#</a> 2.1阻塞IO和非阻塞IO</h3>
<p>那什么是阻塞式 IO，什么是非阻塞 IO 呢？</p>
<p><strong>阻塞IO：</strong></p>
<p>在这种模型中，I/O 操作是阻塞的，即执行 I/O 操作时，线程会被阻塞，直到操作完成。在阻塞 I/O 模型中，每个连接都需要一个线程来处理。因此，对于大量并发连接的场景，阻塞 I/O 模型的性能较差。</p>
<p>在阻塞IO模型中，常见的阻塞操作包括从输入流中读取数据和向输出流中写入数据。例如，在读取文件内容时，程序会一直等待文件数据加载到内存中才能继续执行后续代码。同样地，向网络连接发送数据时，程序会一直等待数据发送完毕才能进行下一步操作。</p>
<p>阻塞IO的特点是简单直观，代码易于理解和编写。然而，它也存在一些潜在的问题。首先，如果IO操作时间较长（如读取大文件或网络延迟高），那么程序将会长时间地处于阻塞状态，影响整体的性能和响应性能。其次，阻塞IO模型通常是单线程的，即一次只能处理一个IO请求，这可能造成资源的浪费和效率低下。</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/Leo100/202311031338172.png" alt="image-20231103133810095" tabindex="0" loading="lazy"><figcaption>image-20231103133810095</figcaption></figure>
<p><strong>非阻塞IO：</strong></p>
<p>为了解决阻塞IO模型的问题，Java引入了非阻塞IO（NIO）机制，使得程序能够在等待IO操作完成时继续执行其他任务，提高了系统的并发性能和响应速度。非阻塞IO模型使用选择器**(Selector)<strong>来管理多个通道</strong>(Channel)**，并通过轮询的方式检查通道上是否有可以进行IO操作的事件，从而实现高效的IO操作。</p>
<p>在这种模型中，I/O 操作不会阻塞线程。当数据尚未准备好时，I/O 调用会立即返回。线程可以继续执行其他任务，然后在适当的时候再次尝试执行 I/O 操作。非阻塞 I/O 模型允许单个线程同时处理多个连接，但可能需要在应用程序级别进行复杂的调度和管理。这种模型通常需要一个专门的线程来处理轮询操作。</p>
<p>**Java NIO(New IO)<strong>库提供了非阻塞IO支持。在NIO中，数据传输通过缓冲区</strong>(Buffer)<strong>进行，而</strong>通道(Channel)**则负责管理缓冲区和底层IO操作。其中，**选择器(Selector)**是实现非阻塞IO的关键组件之一。选择器可以注册多个通道，并监视它们上的IO事件，如读取、写入、连接等。通过轮询选择器上的事件，程序可以快速地响应IO事件，而不会被单个IO操作所阻塞。</p>
<p>与阻塞IO相比，非阻塞IO具有更高的并发性能和响应速度。非阻塞IO模型允许程序同时处理多个IO请求，并且可以在等待IO操作完成时继续执行其他任务，从而充分利用系统资源。然而，使用非阻塞IO也面临着更高的复杂性和编程难度，因为程序需要显式地处理缓冲区和事件轮询等细节。</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/Leo100/202311031341228.png" alt="image-20231103134101127" tabindex="0" loading="lazy"><figcaption>image-20231103134101127</figcaption></figure>
<h3 id="_2-2同步io和非同步io" tabindex="-1"><a class="header-anchor" href="#_2-2同步io和非同步io" aria-hidden="true">#</a> 2.2同步IO和非同步IO</h3>
<p>同步IO的概念是操作系统级别的IO了，若Java程序发起IO请求，若操作系统IO资源未准备好，同步IO不做出任何响应。而非同步IO则会返回一个标记，当IO资源准备完成后，再通过事件机制通知程序。</p>
<p>如下图所示，同步IO会因为IO资源未准备好而进入阻塞状态。</p>
<p>非同步IO则会通过事件机制，避免和操作系统级别的阻塞。</p>
<h2 id="_3-传统的bio通信方式简介" tabindex="-1"><a class="header-anchor" href="#_3-传统的bio通信方式简介" aria-hidden="true">#</a> 3.传统的BIO通信方式简介</h2>
...
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/Leo100/202311031355714.png" alt="image-20231103135551587" tabindex="0" loading="lazy"><figcaption>image-20231103135551587</figcaption></figure>
<h2 id="_4-传统的bio的问题" tabindex="-1"><a class="header-anchor" href="#_4-传统的bio的问题" aria-hidden="true">#</a> 4.传统的BIO的问题</h2>
<ol>
<li>低效率：在等待IO操作时，线程会一直处于阻塞状态，占用系统资源，无法响应其他请求。如果有大量的并发请求，那么线程将被频繁地切换，导致系统性能下降。</li>
<li>编程复杂度高：由于每个连接都需要一个独立的线程来处理，因此需要管理大量的线程。而线程管理是一项非常复杂的任务，容易出错。</li>
<li>可伸缩性差：由于每个连接都需要一个独立的线程来处理，线程数量可能会随着连接数的增加而急剧增加，导致系统崩溃或者性能急剧下降。</li>
<li>不支持异步IO：BIO只支持同步阻塞IO模型，无法实现异步IO，而异步IO在处理高并发和大吞吐量的场景中具有重要意义。</li>
</ol>
<h2 id="_4-bio通信方式深入分析" tabindex="-1"><a class="header-anchor" href="#_4-bio通信方式深入分析" aria-hidden="true">#</a> 4.BIO通信方式深入分析</h2>
<h3 id="_4-1-bio的工作机制" tabindex="-1"><a class="header-anchor" href="#_4-1-bio的工作机制" aria-hidden="true">#</a> 4.1 BIO的工作机制</h3>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/Leo100/202311031405854.png" alt="image-20231103140520774" tabindex="0" loading="lazy"><figcaption>image-20231103140520774</figcaption></figure>
<p><strong>对 BIO 编程流程的梳理</strong></p>
<ol>
<li>服务器端启动一个 ServerSocket，注册端口，调用accpet方法监听客户端的Socket连接。</li>
<li>客户端启动 Socket对服务器进行通信，默认情况下服务器端需要对每个客户 建立一个线程与之通讯</li>
</ol>
<h3 id="_4-2-传统的bio编程实例回顾" tabindex="-1"><a class="header-anchor" href="#_4-2-传统的bio编程实例回顾" aria-hidden="true">#</a> 4.2 传统的BIO编程实例回顾</h3>
<blockquote>
<p>网络编程的基本模型是Client/Server模型，也就是两个进程之<br>
间进行相互通信，其中服务端提供位置信（绑定IP地址和端口），<br>
客户端通过连接操作向服务端监听的端口地址发起连接请<br>
求，基于TCP协议下进行三次握手连接，连接成功后，双方<br>
通过网络套接字（Socket）进行通信。</p>
<p>传统的同步阻塞模型开发中，服务端ServerSocket负责绑定<br>
IP地址，启动监听端口；客户端Socket负责发起连接操作。连<br>
接成功后，双方通过输入和输出流进行同步阻塞式通信。<br>
基于BIO模式下的通信，<strong>客户端 - 服务端</strong>是完全<strong>同步</strong>，完全耦合的。</p>
</blockquote>
<h3 id="_4-3-多发多收案例" tabindex="-1"><a class="header-anchor" href="#_4-3-多发多收案例" aria-hidden="true">#</a> 4.3 多发多收案例</h3>
<h4 id="_1-客户端编写" tabindex="-1"><a class="header-anchor" href="#_1-客户端编写" aria-hidden="true">#</a> 1.客户端编写</h4>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ClientDemo</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">Exception</span> <span class="token punctuation">{</span>
        <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"==客户端的启动=="</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token comment">// （1）创建一个Socket的通信管道，请求与服务端的端口连接。</span>
        ...
        <span class="token comment">// （2）从Socket通信管道中得到一个字节输出流。</span>
        ...
        <span class="token comment">// （3）把字节流改装成自己需要的流进行数据的发送</span>
        ...
        <span class="token comment">// （4）开始发送消息</span>
        ps<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"我是客户端，我想约吃烤肉！！！"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-服务端编写" tabindex="-1"><a class="header-anchor" href="#_2-服务端编写" aria-hidden="true">#</a> 2.服务端编写</h4>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ServerDemo</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">Exception</span> <span class="token punctuation">{</span>
        <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"==服务器的启动=="</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        ...
        <span class="token comment">//（2）开始在这里暂停等待接收客户端的连接,得到一个端到端的Socket管道</span>
        ...
        <span class="token comment">//（3）从Socket管道中得到一个字节输入流。</span>
        ...
        <span class="token comment">//（4）把字节输入流包装成自己需要的流进行数据的读取。</span>
        ...
        <span class="token comment">//（5）读取数据</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_3-小结" tabindex="-1"><a class="header-anchor" href="#_3-小结" aria-hidden="true">#</a> 3.小结</h4>
<blockquote>
<p>在以上通信中，服务端会一致等待客户端的消息，如果客户端没有进行消息的发送，服务端将一直进入阻塞状态。同时服务端是按照行获取消息的，这意味着客户端也必须按照行进行消息的发送，否则服务端将进入等待消息的阻塞状态！</p>
</blockquote>
<h3 id="_4-4-bio模式下接收多个客户端" tabindex="-1"><a class="header-anchor" href="#_4-4-bio模式下接收多个客户端" aria-hidden="true">#</a> 4.4 BIO模式下接收多个客户端</h3>
<p>在上面的案例中，一个服务端只能接收到一个客户端传递过来的请求，那么如果服务端需要处理很多个客户端的消息通信请求应该如何处理呢，此时我们就需要在服务端引入线程了，也就是说客户端每发起一个请求，服务端就创建一个新的线程来处理这个客户端的请求，这样就实现了一个客户端一个线程的模型。</p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/Leo100/202311031414020.png" alt="image-20231103141359894" tabindex="0" loading="lazy"><figcaption>image-20231103141359894</figcaption></figure>
<p>下面我们通过编写代码案例进一步深入。</p>
<h4 id="_1-服务端编写" tabindex="-1"><a class="header-anchor" href="#_1-服务端编写" aria-hidden="true">#</a> 1.服务端编写</h4>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ServerDemo</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
<span class="token keyword">class</span> <span class="token class-name">ServerReadThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-客户端编写" tabindex="-1"><a class="header-anchor" href="#_2-客户端编写" aria-hidden="true">#</a> 2.客户端编写</h4>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token doc-comment comment">/**
    目标: Socket网络编程。
...
<h3 id="_4-4-java-bio模式下的端口转发思想" tabindex="-1"><a class="header-anchor" href="#_4-4-java-bio模式下的端口转发思想" aria-hidden="true">#</a> 4.4 Java BIO模式下的端口转发思想</h3>
<p><strong>需要实现一个客户端的消息可以发送给所有的客户</strong></p>
<figure><img src="https://gaoziman.oss-cn-hangzhou.aliyuncs.com/Leo100/202311031421989.png" alt="image-20231103142118923" tabindex="0" loading="lazy"><figcaption>image-20231103142118923</figcaption></figure>
<h4 id="_1-客户端代码编写" tabindex="-1"><a class="header-anchor" href="#_1-客户端代码编写" aria-hidden="true">#</a> 1.客户端代码编写</h4>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">OutputStream</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">PrintStream</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>net<span class="token punctuation">.</span></span><span class="token class-name">Socket</span></span><span class="token punctuation">;</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Client</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-客户端线程处理类" tabindex="-1"><a class="header-anchor" href="#_2-客户端线程处理类" aria-hidden="true">#</a> 2.客户端线程处理类</h4>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">BufferedReader</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">InputStream</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">InputStreamReader</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>net<span class="token punctuation">.</span></span><span class="token class-name">Socket</span></span><span class="token punctuation">;</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ClientReaderThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_3-服务端代码编写" tabindex="-1"><a class="header-anchor" href="#_3-服务端代码编写" aria-hidden="true">#</a> 3.服务端代码编写</h4>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>net<span class="token punctuation">.</span></span><span class="token class-name">ServerSocket</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>net<span class="token punctuation">.</span></span><span class="token class-name">Socket</span></span><span class="token punctuation">;</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Server</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_4-服务端线程处理类" tabindex="-1"><a class="header-anchor" href="#_4-服务端线程处理类" aria-hidden="true">#</a> 4.服务端线程处理类</h4>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">BufferedReader</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">InputStreamReader</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">PrintStream</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>net<span class="token punctuation">.</span></span><span class="token class-name">Socket</span></span><span class="token punctuation">;</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ServerReaderThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="测试" tabindex="-1"><a class="header-anchor" href="#测试" aria-hidden="true">#</a> 测试</h4>
<p>先启动服务端再启动客户端</p>
<div class="language-markdown line-numbers-mode" data-ext="md"><pre v-pre class="language-markdown"><code>服务器收到消息：大家好，我是客户端一
服务器收到消息：哈哈哈哈
...
<span class="token title important"><span class="token punctuation">#</span>客户端一</span>
大家好，我是客户端一 --发送
哈哈哈哈 --发送
大家好，我是client2 --接收
嘻嘻嘻嘻 --接收
...
<span class="token title important"><span class="token punctuation">#</span>客户端二</span>
大家好，我是客户端一 --接收
哈哈哈哈 --接收
大家好，我是client2 --发送
嘻嘻嘻嘻 --发送
...
<span class="token title important"><span class="token punctuation">#</span>客户端三</span>
大家好，我是客户端一 --接收
哈哈哈哈 --接收
大家好，我是client2 --接收
嘻嘻嘻嘻 --接收
hello everyone --发送
i am client3 --发送
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_5-总结" tabindex="-1"><a class="header-anchor" href="#_5-总结" aria-hidden="true">#</a> 5.总结</h2>
