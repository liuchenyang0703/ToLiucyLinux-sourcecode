<template><div><figure><img src="https://lcy-blog.oss-cn-beijing.aliyuncs.com/blog/202412180945178.jpeg" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<h2 id="_1、tomcat整体架构" tabindex="-1"><a class="header-anchor" href="#_1、tomcat整体架构" aria-hidden="true">#</a> 1、Tomcat整体架构</h2>
<p><img src="https://lcy-blog.oss-cn-beijing.aliyuncs.com/blog/202412180945033.png" alt="" loading="lazy"><br>
...
<img src="https://lcy-blog.oss-cn-beijing.aliyuncs.com/blog/202412180945706.png" alt="" loading="lazy"><br>
...
<h2 id="_2、tomcat优化" tabindex="-1"><a class="header-anchor" href="#_2、tomcat优化" aria-hidden="true">#</a> 2、Tomcat优化</h2>
<h3 id="_2-1-jvm内存优化" tabindex="-1"><a class="header-anchor" href="#_2-1-jvm内存优化" aria-hidden="true">#</a> 2.1 JVM内存优化</h3>
...
<figure><img src="https://lcy-blog.oss-cn-beijing.aliyuncs.com/blog/202412180945437.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<blockquote>
...
<blockquote>
<p><strong><font color=bluesa>补充说明：</font></strong></p>
<ol>
<li>JVM中的堆和栈<br>
栈解决程序的运行问题，即程序如何执行，或者说如何处理数据；堆解决的是数据存储的问题，即数据怎么放、放在哪儿。在Java中一个线程就会相应有一个线程栈与之对应，这点很容易理解，因为不同的线程执行逻辑有所不同，因此需要一个独立的线程栈。而堆则是所有线程共享的。栈因为是运行单位，因此里面存储的信息都是跟当前线程（或程序）相关信息的。包括局部变量、程序运行状态、方法返回值等等；而堆只负责存储对象信息。<br></li>
<li>OOM<br>
OOM(Out of Memory)内存溢出，OOM发生的主要原因是，第一次物理内存被使用的时候，物理内存中其实没有分配（次页缺失），恰好系统无法释放物理内存，造成系统内存不够，导致系统会自动kill掉一些进程来释放物理内存。OOM会选择占用内存最多的进程开始杀，一直到内存足够。</li>
</ol>
</blockquote>
<h3 id="_2-2-jvm内存调整" tabindex="-1"><a class="header-anchor" href="#_2-2-jvm内存调整" aria-hidden="true">#</a> 2.2 JVM内存调整</h3>
...
<figure><img src="https://lcy-blog.oss-cn-beijing.aliyuncs.com/blog/202412180945612.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="https://lcy-blog.oss-cn-beijing.aliyuncs.com/blog/202412180945968.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<h3 id="_2-3-jvm相关监控-了解" tabindex="-1"><a class="header-anchor" href="#_2-3-jvm相关监控-了解" aria-hidden="true">#</a> 2.3 JVM相关监控（了解）</h3>
<h4 id="_2-3-1-jps" tabindex="-1"><a class="header-anchor" href="#_2-3-1-jps" aria-hidden="true">#</a> 2.3.1 jps</h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-3-2-jstat" tabindex="-1"><a class="header-anchor" href="#_2-3-2-jstat" aria-hidden="true">#</a> 2.3.2 jstat</h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-3-3-jmap" tabindex="-1"><a class="header-anchor" href="#_2-3-3-jmap" aria-hidden="true">#</a> 2.3.3 jmap</h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-3-4-jstack" tabindex="-1"><a class="header-anchor" href="#_2-3-4-jstack" aria-hidden="true">#</a> 2.3.4 jstack</h4>
...
<h4 id="_2-3-5-jhat" tabindex="-1"><a class="header-anchor" href="#_2-3-5-jhat" aria-hidden="true">#</a> 2.3.5 jhat</h4>
...
<h4 id="_2-3-6-手动编写内存监控界面" tabindex="-1"><a class="header-anchor" href="#_2-3-6-手动编写内存监控界面" aria-hidden="true">#</a> 2.3.6 手动编写内存监控界面:</h4>
...
<h4 id="_2-4-压缩传输" tabindex="-1"><a class="header-anchor" href="#_2-4-压缩传输" aria-hidden="true">#</a> 2.4 压缩传输</h4>
...
<font color=redsal>补充说明:</font><p>  Tomcat 的压缩是在客户端请求服务器对应资源后，从服务器端将资源文件压缩，再输出到客户端，由客户端的浏览器负责解压缩并浏览。相对于普通的浏览过程HTML、CSS、Javascript和Text，它可以节省40% 左右的流量。更为重要的是，它可以对动态生成的，包括CGI、PHP、JSP、ASP、Servlet,SHTML等输出的网页也能进行压缩，压缩效率也很高。但是，压缩会增加Tomcat的负担，因此最好采用Nginx+ Tomcat或者Apache+Tomcat方式，将压缩的任务交由Nginx/Apache去做。</p>
</blockquote>
<h4 id="_2-5-并发连接调优" tabindex="-1"><a class="header-anchor" href="#_2-5-并发连接调优" aria-hidden="true">#</a> 2.5 并发连接调优</h4>
...
<p><strong><font color=redsal>补充说明:</font></strong><br>
  maxThreads是Tomcat所能接受最大连接数。一般设置不要超过8000以上，如果网站访问量非常大，可以使用运行多个Tomcat实例的方法，即在一个服务器上启动多个tomcat然后做负载均衡处理。这里还需要注意的一点是，tomcat和php不同。php可以按照cpu和内存的情况去配置连接数，上万很正常。而java还需要注意jvm的参数配置。如果不注意就会因为jvm参数过小而崩溃。</p>
</blockquote>
...
<blockquote>
<p><strong><font color=redsal>补充说明：</font></strong><br><br>
<strong><font color=order>CPU密集型（CPU-bound）</font></strong><br>
  CPU密集型也叫计算密集型，指的是系统的硬盘、内存性能相对CPU要好很多，此时，系统运作大部分的状况是CPU Loading 100%，CPU要读/写I/O(硬盘/内存)，I/O在很短的时间就可以完成，而CPU还有许多运算要处理，CPU Loading很高。<br>
  在多重程序系统中，大部份时间用来做计算、逻辑判断等CPU动作的程序称之CPU bound。例如一个计算圆周率至小数点一千位以下的程序，在执行的过程当中绝大部份时间用在三角函数和开根号的计算，便是属于CPU bound的程序。<br>
  CPU bound的程序一般而言CPU占用率相当高。这可能是因为任务本身不太需要访问I/O设备，也可能是因为程序是多线程实现因此屏蔽掉了等待I/O的时间。<br><br>
<strong><font color=order>IO密集型（I/O bound）</font></strong><br>
...
<strong><font color=order>CPU密集型 vs IO密集型</font></strong><br>
...
<h4 id="_2-6-禁用dns查询" tabindex="-1"><a class="header-anchor" href="#_2-6-禁用dns查询" aria-hidden="true">#</a> 2.6 禁用DNS查询</h4>
...
<h4 id="_2-7-io优化" tabindex="-1"><a class="header-anchor" href="#_2-7-io优化" aria-hidden="true">#</a> 2.7 IO优化</h4>
<p>IO优化指的是调整Connector运行模式，可以参考Connector的运行模式进行调整。</p>
<h4 id="_2-8-添加listener" tabindex="-1"><a class="header-anchor" href="#_2-8-添加listener" aria-hidden="true">#</a> 2.8 添加Listener</h4>
...
<p><strong><font color=redsal>补充说明：</font></strong><br></p>
...
<h4 id="_2-9-版本号隐藏" tabindex="-1"><a class="header-anchor" href="#_2-9-版本号隐藏" aria-hidden="true">#</a> 2.9 版本号隐藏</h4>
